...
- 修复封面索引并发写入覆盖，增加并发保存回归测试。
- 修复 zone 回退后的显示和控制目标不一致。
- 增加 SIGTERM、SIGINT 和致命异常的统一退出流程。
- 删除不可达旧入口、旧前端资源、jQuery、无效时钟/内存清理和重复 Roon zone 缓存。
- 精简浏览器键盘控制与 Express 中间件，修复小图库随机更新请求无效图片的问题。
- 升级生产依赖，npm audit --omit=dev 报告 0 个漏洞。
